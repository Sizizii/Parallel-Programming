...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    if ((h_out < Height_out) && (w_out < Width_out)){    
      ...
      for (int p = 0; p < K; p++){
        for (int q = 0; q < K; q++){
          acc += shared_3d(c, threadIdx.y + p, threadIdx.x + q) * mask_4d(m, c, p, q);
        }
      }
      atomicAdd((&out_4d(b, m, h_out, w_out)), acc);
    }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    conv_forward_kernel <<< gridDim, blockDim, Channel * shared_w * shared_w * sizeof(float) >>> (device_output, device_input, device_mask, Batch, Map_out, Channel, Height, Width, K);
