...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    extern __shared__ float Shared_in[];  //cover all input
    ...
    #define shared_3d(i2, i1, i0) Shared_in[(i2) * (TILE_WIDTH * Channel) + (i1) * Channel + i0] //read from shared memory
    ...
    if ((h_out < Height_out) && (w_out < Width_out)){    
      ...
      for (int p = 0; p < K; p++){
        for (int q = 0; q < K; q++){
          acc += in_4d(b, c, h_out + p, w_out + q) * mask_4d(m, c, p, q);
        }
      }
      shared_3d(threadIdx.y, threadIdx.x, c) = acc;

      for (unsigned int stride = 1; stride <= blockDim.z; stride *= 2)
      {
        __syncthreads();
        if ((c % stride == 0) && (2*c + stride < blockDim.z))
        {
          shared_3d(threadIdx.y, threadIdx.x, 2*c) += shared_3d(threadIdx.y, threadIdx.x, 2*c + stride);
        }
      ...
      if (c == 0) 
      {
        out_4d(b, m, h_out, w_out) = shared_3d(threadIdx.y, threadIdx.x, c);
      }
    }
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    CUDA_ERROR(cudaMemcpyToSymbol(Mc, host_mask, size_Kernel*sizeof(float), 0, cudaMemcpyHostToDevice));
}
