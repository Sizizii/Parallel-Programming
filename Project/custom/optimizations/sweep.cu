    ...
    */
    ...
    if ((h < Height_out) && (w < Width_out)){
      ...
      for (int c = 0; c < Channel; c++){
        for (int p = 0; p < K; p++){
              acc += in_4d(b, c, h + p, w ) * mask_4d(m, c, p, 0);
              acc += in_4d(b, c, h + p, w + 1) * mask_4d(m, c, p, 1);
              acc += in_4d(b, c, h + p, w + 2) * mask_4d(m, c, p, 2);
              acc += in_4d(b, c, h + p, w + 3) * mask_4d(m, c, p, 3);
              acc += in_4d(b, c, h + p, w + 4) * mask_4d(m, c, p, 4);
              acc += in_4d(b, c, h + p, w + 5) * mask_4d(m, c, p, 5);
              acc += in_4d(b, c, h + p, w + 6) * mask_4d(m, c, p, 6);
        }
      }
      out_4d(b, m, h, w) = acc;
    }
